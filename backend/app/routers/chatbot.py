...
async def send_message(
    ...
    if not message:
        return ChatResponse(
            ...
            suggestions=["What should I pack?", "Best time to visit?", "Budget breakdown"],
        )
    ...
    if req.trip_id and user:
        ...
        if trip:
            trip_info = {
                ...
                "budget": str(trip.total_budget),
            }

    history = [{"role": m.role, "content": m.content} for m in req.conversation_history]
    result = await chat_with_gemini(message, history, dest_name, trip_info)
