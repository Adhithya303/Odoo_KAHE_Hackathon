...
export default function ChatWindow() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: "Hi! I'm your WanderIQ travel assistant. Ask me about destinations, visa basics, packing tips, budgets, food, or safety.",
    },
  ]);
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] bg-white rounded-card shadow-elevated border border-border animate-slide-up overflow-hidden">
          <div className="bg-primary p-4 flex items-center gap-3">
            <div className="w-9 h-9 bg-white/20 rounded-full flex items-center justify-center text-sm font-semibold">AI</div>
            <div>
                ...
                <div
                  className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${
                    msg.role === 'user'
                      ...
                      : 'bg-white text-body shadow-sm rounded-bl-md border border-border'
                  }`}
                >
                  {msg.content}
                </div>
