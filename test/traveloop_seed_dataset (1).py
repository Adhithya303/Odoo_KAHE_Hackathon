...
def seed(csv_path: str, config: dict):
    ...
    conn.commit()
    print(f"  ✔ {cursor.rowcount} rows inserted into recommendation_profiles")
    ...
    conn.commit()
    print(f"  ✔ Updated {len(dest_stats)} destination budget averages")
    ...
    conn.commit()
    print(f"  ✔ {len(dt_pairs)} destination–trip_type pairs")
    ...
    conn.commit()
    print(f"  ✔ {len(dg_pairs)} destination–group_type pairs")
    ...
    conn.commit()
    print(f"  ✔ {len(dm_pairs)} destination–travel_month pairs")
